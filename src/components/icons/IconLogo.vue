<!-- FlowerIcon.vue -->
<template>
  <svg :width="size" :height="size" :fill="color" :style="{ transform: `rotate(${rotation}deg)` }" viewBox="0 0 512 512"
    xmlns="http://www.w3.org/2000/svg" v-bind="$attrs">
    <title>{{ title }}</title>
    <path v-for="(path, index) in paths" :key="index" :d="path" :style="getPathStyle(index)" />
  </svg>
</template>
...
const getPathStyle = (index: number) => {
  return props.pathColors[index] ? { fill: props.pathColors[index] } : {};
};
</script>
