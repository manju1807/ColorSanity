...
const backgroundStyle = computed(() => {
  ...
  if (isGradient.value) {
    ...
    const colorStops = colors
      .map((color: string) => applyOpacityToColor(color, opacity ?? 1))
      .join(", ");
...
// Reactive props for popover positioning
const popoverSide = ref<"left" | "right">("right");
const popoverAlign = ref<"start" | "center" | "end">("start");
const popoverSideOffset = ref(5);

// Function to update popover position based on screen size
const updatePopoverPosition = () => {
  if (window.innerWidth < 768) {
    popoverSide.value = "left"; // Position popover to the left on small screens
    popoverAlign.value = "start";
    popoverSideOffset.value = 10; // Add some spacing
  } else {
    popoverSide.value = "right"; // Default to right on larger screens
    popoverAlign.value = "start";
    popoverSideOffset.value = 5;
  }
};
  ...
  <div class="relative group rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow bg-white">
    ...
    <div id="controls" class="absolute top-0 right-0 p-4 flex gap-2">
      ...
      <Popover>
        ...
        <PopoverContent class="w-80 max-w-[90vw] p-4 bg-white shadow-lg rounded-lg" :side="popoverSide"
          :align="popoverAlign" :sideOffset="popoverSideOffset" :collisionPadding="10">
          <div class="space-y-4">
