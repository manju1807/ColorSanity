...
import { Moon, Paintbrush, Palette, RotateCcw, Sun } from "lucide-vue-next";
  ...
  <nav
    ...
    <div class="container flex h-14 items-center justify-between px-4">
      <div class="flex items-center gap-2 cursor-pointer" @click="router.replace('/')">
        <IconLogo color="hsl(var(--primary))" />
        <span class="font-bold text-primary tracking-tight">Gradio</span>
      </div>
      <div class="hidden md:flex items-center space-x-4">
        <Button v-for="item in NavItems" :key="item.label" variant="ghost" @click="router.push(item.navRoute)"
          :class="{ 'text-primary': router.currentRoute.value.path === item.navRoute }">
          {{ item.label }}
        </Button>
      </div>
                    ...
                    <div>
                      ...
                      <div class="mt-2 grid grid-cols-3 gap-2">
                        <Button v-for="preset in presets" :key="preset" variant="outline" size="sm"
                          class="justify-start gap-2" @click="handlePresetChange(preset)">
                  ...
                  <div class="space-y-4">
                    <div>
                      ...
                      <Select :model-value="currentFontFamily" @update:model-value="handleFontFamilyChange">
                        ...
                        <SelectContent>
                          <SelectItem v-for="font in fontFamilyOptions" :key="font" :value="font">{{ font }}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    <Card>
                      ...
                      <CardContent class="space-y-4">
                        <p class="text-2xl font-semibold">The quick brown fox jumps over the lazy dog</p>
                        <p class="text-sm text-muted-foreground">Font: {{ currentFontFamily }}</p>
                      </CardContent>
                    </Card>
                  </div>
                </TabsContent>
